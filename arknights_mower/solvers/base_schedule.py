...
class BaseSchedulerSolver(BaseSolver):
    ...
    def run(self, arrange: dict[str, tp.BasePlan] = None, clue_collect: bool = False, drone_room: str = None,
            fia_room: str = None) -> None:
        """
        ...
        plan_1 = {
            ...
            'central': [{'agent': '焰尾', 'group': '红松骑士', 'replacement': ['战车']},
                        {'agent': '琴柳', 'group': '迷迭香', 'replacement': ['霜华']},
                        {'agent': '凯尔希', 'replacement': ['灰烬']},
                        {'agent': '夕', 'group': '迷迭香', 'replacement': ['闪击'],
                         'moodLimit': 13},
                        {'agent': '令', 'group': '迷迭香', 'replacement': ['闪击']},
                        ],
            'contact': [{'agent': '絮语', 'group': '迷迭香', 'replacement': ['艾雅法拉']}],
            # 宿舍
            ...
            'dormitory_1': [{'agent': '波登可', 'time': ''},
                            {'agent': '夜莺', 'time': ''},
                            {'agent': '菲亚梅塔', 'replacement': ['巫恋', '迷迭香', '黑键']}],
            # 会客室
            'meeting': [{'agent': '陈', 'replacement': ['星极']},
                        {'agent': '红', 'replacement': ['远山']}, ],
            'room_1_1': [{'agent': '巫恋', 'group': '巫恋', 'replacement': ['菲亚梅塔']},
                         {'agent': '卡夫卡', 'group': '', 'Type': '', 'replacement': ['柏喙']},
                         {'agent': '黑键', 'replacement': ['菲亚梅塔']}],
            ...
            'room_3_1': [{'agent': '异客', 'group': '异客', 'replacement': ['菲亚梅塔']},
                         {'agent': '森蚺', 'group': '异客', 'Type': '', 'replacement': ['柏喙']},
                         {'agent': '温蒂', 'group': '异客', 'time': '', 'replacement': ['炎狱炎熔']}],
            'room_3_2': [{'agent': '食铁兽', 'group': '', 'replacement': ['菲亚梅塔']},
                         {'agent': '断罪者', 'group': '', 'Type': '', 'replacement': ['白雪']},
                         {'agent': '白雪', 'group': '', 'time': '', 'replacement': ['炎狱炎熔']}],
            'room_3_3': [{'agent': '澄闪', 'group': '', 'time': '', 'replacement': ['炎狱炎熔']}]
        }
        currentBase = {
            'central': [{'agent': '焰尾', 'mood': 12},
                        {'agent': '琴柳', 'mood': 24},
                        {'agent': '凯尔希', 'mood': 24, },
                        {'agent': '夕', 'mood': 10},
                        {'agent': '令', 'mood': 24, }]
        ...
        for key in currentBase:
            for idx, operator in enumerate(currentBase[key]):
                totalAgent.append(currentBase[key][idx])
                if "group" in plan_1[key][idx].keys():
                    print(plan_1[key][idx]["group"])
                # operator.group = plan_1[key][idx].group
        totalAgent.sort(key=lambda x: x["mood"], reverse=True)
        print(totalAgent)
            ...
            for block in ret:
                if base_room_list[idx] in need_empty:
                    ...
                    while not finished:
                        ...
                        # 进入房间
                        abc = (block[2][0] - block[0][0]) // 7 + block[0][0]
                        self.tap((abc, y))
                        self.tap((x, y))
