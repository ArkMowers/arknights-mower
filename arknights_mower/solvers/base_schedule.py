...
#         return None, None
    ...
    def get_run_roder_time(self, room):
        ...
        logger.info(room + ' 接单时间为：' + execute_time.strftime("%H:%M:%S"))
...
#         return clues
    ...
    def drone(self, room: str, not_customize=False, not_return=False):
        ...
        while accelerate:
            logger.info('贸易站加速')
            self.tap(accelerate)
            for i in range(self.用于拉开订单时间差距的无人机数量):
...
#                     agents[idx] = 'Free'
        ...
        if ('龙舌兰' in agent) or ('但书' in agent):
            logger.warning(room + ' 龙舌兰、但书的进驻时间为：' + (self.tasks[0].time + timedelta(seconds=(self.跑单提前运行时间 - self.更换干员前缓冲时间))).strftime("%H:%M:%S"))
#         # 若不是空房间，则清空工作中的干员
...
#                             new_plan[working_room] = self.op_data.get_current_room(working_room, True)
                    ...
                    self.recog.update()
                    if len(new_plan) == 1:
                        龙舌兰_但书进驻前的等待时间 = (self.tasks[0].time - datetime.now()).total_seconds() + self.跑单提前运行时间 - self.更换干员前缓冲时间
                        if 龙舌兰_但书进驻前的等待时间 > 0:
