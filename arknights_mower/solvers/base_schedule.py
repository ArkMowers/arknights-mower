...
class BaseSchedulerSolver(BaseSolver):
            ...
            for i in range(1, 8):
                ...
                if len(ori_results) == 3:
                    ...
                    for swiptimes in range(1,3):
                        self.swipe((self.recog.w * 0.8, self.recog.h * 0.8), (0, -self.recog.h * 0.15),rebuild=False)
                    self.recog.update()
    ...
    def get_agent_from_room(self, room, read_time_index=None):
        ...
        if length > 3: self.swipe((self.recog.w * 0.8, self.recog.h * 0.8), (0, self.recog.h * 0.15), interval=1,
                                  rebuild=True)
        ...
        for i in range(0, length):
            if i >= 3 and not swiped:
                self.swipe((self.recog.w * 0.8, self.recog.h * 0.8), (0, -self.recog.h * 0.15), interval=1, rebuild=True)
                swiped = True
            data = {}
    ...
    def agent_arrange(self, plan: tp.BasePlan, metadata=None):
        ...
        if len(new_plan) == 1:
            ...
            self.back(interval=0.5)
            self.tasks.append({'time': self.tasks[0]['time'], 'plan': new_plan})
