...
class BaseSchedulerSolver(BaseSolver, BaseMixin):
        ...
        if len(self.op_data.run_order_rooms) > 0:
            ...
            if valid:
                ...
                for k, v in self.op_data.run_order_rooms.items():
                    if find_next_task(self.tasks, meta_data=k) is not None:
                        continue
                    if not valid:
                        continue
                    ...
                    self.tasks.append(
                        SchedulerTask(
                            time=self.get_run_roder_time(k),
    ...
    def share_clue(self):
        ...
        for i in range(1, 8):
            ...
            if len(ori_results) > 1:
                ...
                self.tap(
                    (
                        (last_ori[0][0] + last_ori[2][0]) / 2,
                        (last_ori[0][1] + last_ori[2][1]) / 2,
                    ),
                    interval=1,
                )
    ...
    def clue(self) -> None:
        ...
        if self.maa_config["maa_enable"] and self.party_time is not None:
            if find_next_task(self.tasks, task_type=TaskTypes.MAA_MALL) is None:
                ...
                self.tasks.append(
                    SchedulerTask(time=self.party_time, task_type=TaskTypes.MAA_MALL)
                )
    ...
    def recruit_plan_solver(self):
        if (
            ...
            logger.info(
                "下一次公开招募执行时间在{}小时之后".format(
                    self.recruit_config["recruit_execution_gap"]
                )
            )
