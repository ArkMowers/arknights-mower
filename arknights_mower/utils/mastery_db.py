...
def get_route_settings(path: Optional[str] = None) -> dict:
    ...
    缺行使用 10/15/30 分钟；旧默认单值 10 一次性迁移为分档默认，其他旧值保留。
    """
    defaults = {
        **_SETTINGS_DEFAULTS,
            ...
            if row:
                ...
                if isinstance(parsed, dict):
                    ...
                    if isinstance(parsed.get("mastery_swap_buffers"), dict):
                        ...
                        legacy = parsed["mastery_swap_buffer"]
                        defaults["mastery_swap_buffers"] = (
                            dict(DEFAULT_SWAP_BUFFERS)
                            if legacy == DEFAULT_SWAP_BUFFER_MINUTES
                            else {key: legacy for key in DEFAULT_SWAP_BUFFERS}
                        )
