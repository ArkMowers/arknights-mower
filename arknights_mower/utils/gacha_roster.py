...
def roster_preview(base: Path | None = None) -> dict:
    base = Path(base) if base is not None else Path(os.environ.get("MOWER_DATA_DIR", "."))
    ...
    if not cache.is_file():
        return {"available":False,"message":"尚无森空岛干员缓存；请在 Mower 中手动同步森空岛干员数据。"}
    if not meta_file:
    ...
    if not isinstance(chars,list) or not chars:
        return {"available":False,"message":"当前缓存没有有效的森空岛干员列表，请先在 Mower 中同步。"}
    ...
    result.sort(key=lambda a:(-(a["rarity"] if isinstance(a["rarity"],int) else 0),a["name"]))
    return {
        "available":True,"source":"current-device-skland-cache",
