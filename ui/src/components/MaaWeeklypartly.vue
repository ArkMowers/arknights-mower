  ...
  <n-card>
    ...
    <n-table size="small" :single-line="false" style="text-align: center">
      ...
      <tbody>
        ...
        <tr>
          <td>
            <n-select v-model:value="map_selete" :options="options" />
          </td>
          <td v-for="day in daysOfWeek" :key="day">
            <span v-for="plan in maa_weekly_plan1" :key="plan.id">
              <div v-if="plan.stage.includes(map_selete) && plan[day] !== 0">
                <n-checkbox
                  v-model="plan[day]"
                  :checked="plan[day] === 2"
                  @change="updatePlan(plan, day)"
                />
              </div>
            </span>
          </td>
