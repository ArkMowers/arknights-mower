...
async function saveTasks(){
  ...
  const task = {
    time : new Date(task_time.value).toLocaleString(), plan,task_type:task_type.value,meta_data:'' 
  }
...
<template>
  <n-modal v-model:show="show" preset="card" title="任务" transform-origin="center" style="width: 900px">
    <div v-if="isLogPage" class="task_row">
      ...
      <label>任务触发时间</label>
      <n-date-picker v-model:value="task_time" type="datetime" placeholder="选择时间"
        style="width: 200px;" />
      <label>任务类别</label>
      <n-select v-model:value="task_type" :options="taskTypeOptions" placeholder="任务类别" class="dropdown-select"
        style="width: 150px;">
      </n-select>
      <label v-if="task_type == '技能专精'">专精技能</label>
      <n-input-number v-if="task_type == '技能专精'" v-model:value="skill_level" :min="1" :max="3"
        style="width: 75px;" />
    </div>
    <n-scrollbar v-if="(isLogPage && task_type != '技能专精') || !isLogPage" style="max-height: 80vh;margin-top: 8px;">
          ...
          <div class="task_row">
            ...
            <n-dynamic-tags v-model:value="value.operators" :max="3" size="large">
              <template #input="{ submit, deactivate }">
                <n-select v-model:value="value.operators" filterable :options="operators_with_free_current"
                  :on-update:value="(v) => {
                    submit(v)
                  }
                    " :on-blur="deactivate" :filter="(p, o) => match(o.label, p)" :render-label="render_op_label" />
              </template>
            </n-dynamic-tags>
          </div>
        </template>
      ...
      <n-card style="margin-top: 8px" content-style="padding: 8px" embedded>
        <n-code v-if="!isLogPage" :code="JSON.stringify(backup_plans[sub_plan].task, null, 2)" language="json"
          word-wrap />
        <n-code v-if="isLogPage" :code="JSON.stringify(task_list, null, 2)" language="json" word-wrap />
      </n-card>
    </n-scrollbar>
    <n-scrollbar v-if="isLogPage && task_type == '技能专精'" style="max-height: 80vh;margin-top: 8px;">
      <n-dynamic-input v-model:value="upgrade_support" :on-create="new_support" :max="3">
        <template #create-button-default>添加专精工具人</template>
        <template #default="{ value }">
          <div class="task_row">
            <label>工具人</label>
            <n-select v-model:value="value.name" filterable :options="operators_with_free_current"
              :filter="(p, o) => match(o.label, p)" :render-label="render_op_label" style="width: 175px;" />
            <label>等级</label>
            <n-input-number v-model:value="value.skill_level" :min="1" :max="3" style="width: 80px;" />
            <label>效率</label>
            <n-input-number v-model:value="value.efficiency" :min="30" :max="100" style="width: 80px;" />
            <label>替换</label>
            <help-text>
              <div>请选择小鸟/狗剩</div>
              <div>没有的话，请设定和工具人相同</div>
            ...
            <n-select v-model:value="value.swap_name" filterable :options="operators_with_free_current"
              :filter="(p, o) => match(o.label, p)" :render-label="render_op_label" style="width: 175px;" />
          </div>
        </template>
      </n-dynamic-input>
