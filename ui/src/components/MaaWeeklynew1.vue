...
<template>
  <n-card>
    <n-checkbox v-model:checked="maa_enable" class="card-title">
      Maa周计划
    </n-checkbox>
    ...
    <n-table size="small" :single-line="false" style="text-align: center">
      <thead>
        <tr>
          <th style="width: 6%;">关卡</th>
          <th v-for="(day, index) in daysOfWeek" :key="index" :class="{ today: currentDay === index }" style="width: 2%;">
            {{ day }}{{ currentDay === index ? ' (今天)' : '' }} </th>
        </tr>
      </thead>
      <tbody>
        <td>药</td>
        ...
        <tr v-for="(plan1, index) in maa_weekly_plan1" :key="plan1.weekday1">
          <td>
            <n-select v-if="index < 3" v-model:value="plan1.stage" multiple filterable tag :show="false"
          ...
          <td v-for="day in daysOfWeek"
            :class="{ 'class2': plan1[day] === 2, 'class1': plan1[day] === 1, 'class0': plan1[day] === 0 }">
            <template v-if="plan1[day] !== 0">
              <n-checkbox @click="updateStage" v-model="plan1[day]" :checked="plan1[day] === 2"
