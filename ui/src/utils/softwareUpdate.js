...
export function confirmSourceVersion(dialogs, target, instanceCount, install) {
  return dialogs.warning({
    ...
    content: `${target.source_repo ? `将使用仓库 ${target.source_repo}${target.source_pr ? ` 的 PR #${target.source_pr}` : ''}。` : ''}${target.base_commit ? `将以目标分支 ${target.source_branch} 的 ${target.base_commit} 合并所选 PR。` : ''}${target.source_pr ? `PR 提交为 ${target.sha}，合并成功后安装` : `将切换到提交 ${target.sha}`}，重启同一安装目录下的 ${instanceCount} 个实例，重置运行缓存并关闭软件自动更新。配置、专精计划和数据库记录保留；较旧版本可能无法兼容当前配置或恢复任务。${target.force ? '将覆盖本地源码改动，不备份本地修改；即使安装失败，本地修改也不会恢复。' : '安装失败时尝试恢复原版本。'}`,
