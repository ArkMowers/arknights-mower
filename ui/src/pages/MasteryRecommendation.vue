<template>
  <div class="home-container">
    ...
    <n-space style="margin-top: 8px" :size="8" align="center" wrap>
      <n-input v-model:value="searchQuery" placeholder="搜索干员名称" clearable style="width: 200px" size="small" />
      <n-select v-model:value="filterRarity" :options="rarityOptions" multiple placeholder="稀有度" style="min-width: 140px" size="small" clearable />
      <n-select v-model:value="filterProfession" :options="professionOptions" multiple placeholder="职业" style="min-width: 140px" size="small" clearable />
    ...
    <n-alert v-else-if="store.error" type="warning" :closable="false">
      ...
      <div>{{ store.error }}</div>
      <n-button type="primary" size="small" style="margin-top: 12px" @click="fetchCultivate" :loading="store.loading">从森空岛拉取数据</n-button>
    </n-alert>
              ...
              <template #header>
                <n-space align="center" justify="space-between" style="width: 100%">
                  <n-space align="center" :size="8">
                    ...
                    <n-text depth="3" style="font-size: 12px">Lv{{ rec.current_level + 7 }}→10</n-text>
                  </n-space>
                  <n-space :size="4">
                    ...
                    <n-button type="primary" size="tiny" @click.stop="confirmSkill(op, rec)">一键专精</n-button>
                  </n-space>
                </n-space>
              </template>
              <n-space vertical :size="4">
                <n-text depth="2">总训练时间: {{ formatTime(rec.total_time) }} | {{ rec.remaining_levels }}级专精</n-text>
                ...
                <n-grid :x-gap="8" :y-gap="4" cols="3 m:4 l:5 xl:6" responsive="screen">
                  <n-gi v-for="mat in rec.chain_needed_materials" :key="mat.id">
                    <n-thing>
                      <template #avatar>
                        <n-avatar :src="'/depot/' + mat.name + '.webp'" :size="24" fallback-src="/depot/源岩.webp" />
                      </template>
                      <template #header>
                        <n-text :depth="chainHas(rec, mat.id) ? 1 : 3" style="font-size: 11px">{{ mat.name }}</n-text>
                      </template>
                      <template #description>
                        <n-text :type="chainHas(rec, mat.id) ? 'success' : 'error'" style="font-size: 11px">x{{ mat.count }}</n-text>
                      </template>
                    </n-thing>
                  </n-gi>
                </n-grid>
    ...
    <!-- 确认专精 -->
    <n-modal v-model:show="showConfirm" preset="card" title="确认专精任务" style="width: min(560px, 95vw)" :mask-closable="false">
      <n-space vertical>
        <n-text>干员: <n-text strong>{{ cd.op?.name }}</n-text> | <n-tag size="small">{{ professionName(cd.op?.profession) }}</n-tag></n-text>
        <n-text>技能: <n-text strong>{{ cd.rec?.skill_name }}</n-text> → M3 | {{ formatTime(cd.rec?.total_time || 0) }}</n-text>
        <n-divider />
        ...
        <n-divider />
        <n-text :type="cd.rec?.full_chain_achievable ? 'success' : 'warning'">材料: {{ cd.rec?.full_chain_achievable ? '充足 ✓' : '不足 ✗' }}</n-text>
        <n-text v-if="currentMissing(cd.rec).length" style="margin-top: 4px">
          缺少: <n-tag v-for="m in currentMissing(cd.rec)" :key="m.id" type="error" size="small" style="margin-left: 4px">{{ m.name }}x{{ m.count }}</n-tag>
        </n-text>
      </n-space>
      <template #footer>
        <n-space justify="end">
    ...
    <!-- 专精路线设置 -->
    <n-modal v-model:show="showSettings" preset="card" title="专精路线设置" style="width: min(720px, 95vw)" :mask-closable="false">
      <n-tabs type="segment" v-model:value="settingsTab">
        <n-tab-pane v-for="prof in profKeys" :key="prof" :name="prof" :tab="prof">
          <n-scrollbar style="max-height: 60vh">
            <n-dynamic-input v-model:value="routeSettings[prof].supports" :on-create="() => newSupport(prof)" :max="3">
              <template #create-button-default>添加专精工具人</template>
              <template #default="{ value }">
                <div class="support-outer">
                  <n-select v-model:value="value.skill_level" :options="level_list" style="width: 80px" />
                  <div class="support-inner">
                    <div class="task-col">
                      <label style="font-size: 13px">协助位</label>
                      <n-select v-model:value="value.name" filterable :options="operatorOptions" :filter="(p, o) => pinyin_match(o.label, p)" :render-label="render_op_label" style="width: 178px" />
                      <label class="ml" style="font-size: 13px">训练速度</label>
                      <n-input-number v-model:value="value.efficiency" :min="30" :max="100" style="width: 80px" :show-button="false"><template #suffix>%</template></n-input-number>
                    </div>
                    <div class="task-col">
                      <n-checkbox v-model:checked="value.swap">中途换人</n-checkbox>
                      <n-select :disabled="!value.swap" v-model:value="value.swap_name" :options="swap_list" :render-label="render_op_label" style="width: 140px" />
                      <n-select :disabled="!value.swap" v-model:value="value.match" :options="swap_30" style="width: 160px" />
                    </div>
                  </div>
                </div>
              </template>
    ...
    <!-- 专精计划 -->
    <n-modal v-model:show="showPlan" preset="card" title="专精计划" style="width: min(600px, 95vw)" :mask-closable="false">
      <n-space vertical>
        <n-input v-model:value="planSearch" placeholder="搜索干员" clearable size="small" />
        <n-space :size="4" wrap>
          <n-tag v-for="e in planEntries" :key="e.key" closable size="small" type="success" @close="removePlanEntry(e)">
            {{ e.name }} {{ e.skill_name }}
          </n-tag>
        ...
        <n-scrollbar style="max-height: 50vh">
          <div v-for="op in filteredPlanOperators" :key="op.char_id" class="plan-op-row">
            <n-space align="center" :size="4">
              <n-avatar :src="'/avatar/' + op.name + '.webp'" :size="22" round fallback-src="/avatar/阿米娅.webp" />
            ...
            <n-space :size="4" style="margin-left: 8px">
              <n-button
                v-for="rec in op.recommendations" :key="rec.skill_index"
                size="tiny"
                :type="isSkillPlanned(op.char_id, rec.skill_index) ? 'success' : 'default'"
...
const rarityOptions = [{ label: '6★', value: 6 }, { label: '5★', value: 5 }, { label: '4★', value: 4 }, { label: '3★', value: 3 }]
...
function planKey(cid, si) { return `${cid}_${si}` }
function isSkillPlanned(cid, si) { return !!plan.value[planKey(cid, si)] }
function hasPlannedSkill(op) { return op.recommendations.some(r => isSkillPlanned(op.char_id, r.skill_index)) }
function allPlanned(op) { return op.recommendations.every(r => isSkillPlanned(op.char_id, r.skill_index)) }

function toggleSkillPlan(op, rec) {
  const k = planKey(op.char_id, rec.skill_index)
  if (plan.value[k]) { delete plan.value[k] }
  else { plan.value[k] = true }
}
...
function removePlanEntry(e) { delete plan.value[e.key] }
function clearPlan() { plan.value = {} }
function savePlanFn() { localStorage.setItem(PLAN_KEY, JSON.stringify(plan.value)); message.success('计划已保存'); showPlan.value = false }
...
const planEntries = computed(() => {
  ...
  for (const k in plan.value) {
    ...
    if (op) {
      ...
      if (rec) entries.push({ key: k, char_id: cid, skill_index: parseInt(si), name: op.name, skill_name: rec.skill_name })
    }
  }
  return entries
})
...
const swap_list = [{ value: '艾丽妮', label: '艾丽妮' }, { value: '逻各斯', label: '逻各斯' }]
const swap_30 = [{ value: true, label: '有30%速度加成' }, { value: false, label: '无训练速度加成' }]
const level_list = [{ value: 1, label: '专一' }, { value: 2, label: '专二' }, { value: 3, label: '专三' }]

const t60 = { 先锋: { name: '嵯峨', speed: 60 }, 近卫: { name: '史尔特尔', speed: 60 }, 重装: { name: '星熊', speed: 60 }, 狙击: { name: '黑', speed: 60 }, 术师: { name: '卡涅利安', speed: 60 }, 医疗: { name: '阿', speed: 60 }, 辅助: { name: '铃兰', speed: 60 }, 特种: { name: '傀影', speed: 60 } }
const t1 = { 先锋: { name: '夜半', speed: 75 }, 近卫: { name: '赤冬', speed: 75 }, 重装: { name: '极光', speed: 75 }, 狙击: { name: '假日威龙陈', speed: 95 }, 术师: { name: '特米米', speed: 75 }, 医疗: { name: '阿', speed: 60 }, 辅助: { name: '铃兰', speed: 60 }, 特种: { name: '罗宾', speed: 75 } }
const t2 = { 先锋: { name: '缄默德克萨斯', speed: 80 }, 近卫: { name: '燧石', speed: 75 }, 重装: { name: '暴雨', speed: 75 }, 狙击: { name: '埃拉托', speed: 75 }, 术师: { name: '薄绿', speed: 75 }, 医疗: { name: '濯尘芙蓉', speed: 75 }, 辅助: { name: '铃兰', speed: 60 }, 特种: { name: '缄默德克萨斯', speed: 80 } }
const t3 = { 先锋: { name: '嵯峨', speed: 60 }, 近卫: { name: '百炼嘉维尔', speed: 95 }, 重装: { name: '星熊', speed: 60 }, 狙击: { name: 'W', speed: 95 }, 术师: { name: '死芒', speed: 95 }, 医疗: { name: '阿', speed: 60 }, 辅助: { name: '浊心斯卡蒂', speed: 95 }, 特种: { name: '归溟幽灵鲨', speed: 95 } }

const defaultRoute = (p) => ({
  supports: [
    { name: t1[p].name, skill_level: 1, efficiency: t1[p].speed, swap: true, swap_name: ['近卫', '狙击'].includes(p) ? '艾丽妮' : '逻各斯', match: ['近卫', '狙击', '术师', '辅助'].includes(p) },
    { name: t2[p].name, skill_level: 2, efficiency: t2[p].speed, swap: true, swap_name: ['近卫', '狙击'].includes(p) ? '艾丽妮' : '逻各斯', match: ['近卫', '狙击', '术师', '辅助'].includes(p) },
    { name: t3[p].name, skill_level: 3, efficiency: t3[p].speed, swap: true, swap_name: ['近卫', '狙击'].includes(p) ? '艾丽妮' : '逻各斯', match: ['近卫', '狙击', '术师', '辅助'].includes(p) }
  ],
  optimal: false, half_off: true
})
const copyR = (r) => ({ supports: r.supports.map(s => ({ ...s })), optimal: r.optimal, half_off: r.half_off })
const routeSettings = reactive(Object.fromEntries(profKeys.map(p => [p, copyR(defaultRoute(p))])))

function newSupport(p) {
  const n = routeSettings[p].supports.length; if (n >= 3) return null
  const i = n + 1, opt = routeSettings[p].optimal
  const s = opt ? (i === 1 ? t1[p] : i === 2 ? t2[p] : t3[p]) : (i === 3 ? t3[p] : t60[p])
  return { name: s.name, skill_level: i, efficiency: s.speed, swap: true, swap_name: ['近卫', '狙击'].includes(p) ? '艾丽妮' : '逻各斯', match: ['近卫', '狙击', '术师', '辅助'].includes(p) }
}

function loadRoute() {
  try { const d = JSON.parse(localStorage.getItem(ROUTE_KEY) || '{}'); for (const p of profKeys) { if (d[p]) { routeSettings[p].supports = d[p].supports || routeSettings[p].supports; routeSettings[p].optimal = !!d[p].optimal; routeSettings[p].half_off = d[p].half_off !== undefined ? d[p].half_off : true } } } catch {}
}
function saveRoute() { localStorage.setItem(ROUTE_KEY, JSON.stringify(routeSettings)); message.success('已保存'); showSettings.value = false }
function resetRoute() { for (const p of profKeys) Object.assign(routeSettings[p], copyR(defaultRoute(p))); message.info('已恢复默认') }
...
const displayList = computed(() => {
  ...
  if (filterProfession.value.length) list = list.filter(op => filterProfession.value.includes(profMap[op.profession]))
  if (filterAchievable.value) list = list.map(op => ({ ...op, recommendations: op.recommendations.filter(r => r.full_chain_achievable) })).filter(op => op.recommendations.length > 0)
  return list
})
...
function buildSupports(op) {
  const p = profMap[op.profession] || '近卫'; const s = routeSettings[p]
  if (!s?.supports?.length) return []
  return s.supports.map(sup => ({ name: sup.name, swap_name: sup.swap ? (sup.swap_name || sup.name) : sup.name, skill_level: sup.skill_level, efficiency: sup.efficiency || 45, match: sup.swap ? !!sup.match : false, half_off: sup.skill_level === 1 ? s.half_off : false }))
}

function confirmSkill(op, rec) {
  const p = profMap[op.profession] || '近卫'
  cd.op = op; cd.rec = rec; cd.supports = buildSupports(op); cd.firstSupport = routeSettings[p]?.supports?.[0]?.name || ''
...
async function doAddTask() {
  ...
  try {
    const r1 = await axios.post(`${import.meta.env.VITE_HTTP_URL}/task`, { task: { time: new Date(Date.now() + 60000).toISOString(), plan: { train: [firstSupport, op.name] }, task_type: '上班', meta_data: '' } })
    if (r1.data !== '添加任务成功！') { message.warning(r1.data); return }
    const r2 = await axios.post(`${import.meta.env.VITE_HTTP_URL}/task`, { task: { time: new Date(Date.now() + 120000).toISOString(), plan: {}, task_type: '技能专精', meta_data: '' + skillNum }, upgrade_support: supports })
    r2.data === '添加任务成功！' ? message.success(`${op.name} ${rec.skill_name} 专精任务已添加！`) : message.warning(r2.data)
...
.mastery-list { width: 100%; max-width: 960px; }
.rec-item .n-card { margin-bottom: 0; }
.section-label { display: block; margin-bottom: 2px; }
.missing-section { margin-top: 4px; }
.support-outer { margin-bottom: 8px; }
.support-inner { margin-top: 4px; }
.task-col { display: flex; align-items: center; gap: 8px; margin: 4px 0; }
.ml { margin-left: 12px; }
.confirm-support-row { display: flex; align-items: center; gap: 8px; margin: 2px 0; }
.plan-op-row { display: flex; align-items: center; gap: 8px; padding: 4px 0; }
</style>
