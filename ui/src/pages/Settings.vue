              ...
              <n-checkbox v-model:checked="free_room">
                ...
                <help-text>
                  <template v-if="experimental_dorm_logic">
                    <p>
                      有可用候选时，按宿舍优先级和心情补入空床，或替换已满心情、恢复倒计时已结束的动态床位入住者。待命候补也可补床，固定宿舍岗位不清除。
                    </p>
                    <p>
                      恢复中的主班和候补不会被踢出宿舍。新入住者也参与单回竞争，执行后补读恢复时间并更新回班任务；执行时机受任务队列及合并间隔影响。
                    </p>
                  </template>
                  <template v-else>
            ...
            <n-form-item :show-label="false">
              <n-checkbox v-model:checked="experimental_dorm_logic">
                ...
                <help-text>
                  <template v-if="experimental_dorm_logic">
                    <p>
                      当前已开启。排班表中的优先级只决定宿舍分床与单回竞争；下班候选仍按「当前心情减个人心情下限」从低到高尝试。
                    </p>
                    <p>
                      支持绑组宿舍临时
                      Free、候补待命补床和主副表独立房间排序。更高排名的新入住者可触发单回重排；日常保留已有普通床位，避免因心情或宿舍顺序变化反复搬床。
                    </p>
                  </template>
                  <template v-else>
                    <p>
                      当前已关闭，使用稳定版宿舍规则：床位按全局宿舍顺序安排，干员休息优先名单按填写顺序排列；不改变下班候选顺序。
                    </p>
                    <p>
                      开启后可使用按层级和心情分床、候补待命补床、绑组宿舍临时 Free
                      及主副表独立房间排序。此开关默认关闭。
                    </p>
                  </template>
                </help-text>
