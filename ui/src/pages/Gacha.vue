...
const facilityOptions = [
  { label: '全部设施', value: '全部' },
  ...['贸易', '金属', '源石', '通用', '加工', '宿舍', '办公室', '会客室'].map((x) => ({
    label: x,
    value: x
  }))
]
...
const groupOptions = computed(() =>
  availableGroups.value
    .filter(
      (g) => facilityFilter.value === '全部' || (g.facility || '通用') === facilityFilter.value
    )
...
function renderGroupTag({ option, handleClose }) {
  return h(
    ...
    {
      default: () =>
        h('span', { class: 'gacha-option-inner' }, [
          avatarNode(option.value, 24),
          h('span', null, groupOptionName(option.value))
        ])
    }
  )
}
const catalogOptions = computed(() =>
  Object.entries(operatorCatalog.value)
    .filter(([id, x]) => id.startsWith('char_') && x?.name)
    .sort(
      (a, b) =>
        Number(b[1].rarity || 0) - Number(a[1].rarity || 0) ||
...
const groupMembers = computed(() => {
  ...
  return [
    ...activeGroup.value.core.map((id) => [id, 'core']),
    ...
    .map(([id, kind]) => ({
      id,
      kind,
      ...operatorCatalog.value[id],
      name: operatorCatalog.value[id]?.name || id,
...
function groupsStorageKey() {
  return 'mower-gacha-groups-v1-' + (activeId.value || 'unbound')
}
function overridesStorageKey() {
  return 'mower-gacha-group-overrides-v1-' + (activeId.value || 'unbound')
}
function hiddenStorageKey() {
  return 'mower-gacha-group-hidden-v1-' + (activeId.value || 'unbound')
}
function readCustomGroups() {
  try {
    const original = JSON.parse(localStorage.getItem(groupsStorageKey()) || '[]')
    customGroups.value = Array.isArray(original)
      ? original
          .filter(
            (g) =>
              g?.id?.startsWith('custom-') &&
              g.title &&
              Array.isArray(g.core) &&
              Array.isArray(g.support)
          )
          .slice(0, 50)
          .map((g) => ({ ...g, facility: g.facility || '通用', kind: 'custom' }))
      : []
      ...
      overrides && typeof overrides === 'object' && !Array.isArray(overrides)
        ? Object.fromEntries(
            Object.entries(overrides).filter(
              ([id, g]) =>
                builtinGroups.some((original) => original.id === id) &&
                g?.title &&
                Array.isArray(g.core) &&
                Array.isArray(g.support)
            )
...
function persistGroups() {
  try {
    ...
    message('本地存储不可用，组合未保存', 'warning')
  }
}
function startGroup(source = null) {
  const facility =
    source?.facility || (facilityFilter.value === '全部' ? '通用' : facilityFilter.value)
...
function resolveImportMember(item) {
  ...
  if (typeof id === 'string' && operatorCatalog.value[id]) {
    if (typeof name === 'string' && name !== id && operatorCatalog.value[id].name !== name) {
      throw new Error('干员 ID 与名称不匹配：' + name)
    }
    return id
  }
  if (typeof name !== 'string' || !name.trim()) throw new Error('存在空干员名称')
  const matches = Object.entries(operatorCatalog.value).filter(
    ([key, v]) => key.startsWith('char_') && v?.name === name.trim()
  )
  if (matches.length !== 1) {
...
function validateImportedGroup(raw, index) {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw))
    throw new Error('第 ' + (index + 1) + ' 组不是对象')
  ...
  const support = [
    ...new Set((raw.support || []).map(resolveImportMember).filter((id) => !core.includes(id)))
  ]
...
async function readGroupImport(event) {
  ...
  try {
    ...
    const groupsInput =
      json.format === 'mower-gacha-groups-v2'
        ? [
            ...builtinGroups
              .filter((g) => !(json.hidden || []).includes(g.id))
              .map((g) => json.overrides?.[g.id] || g),
            ...json.groups
          ]
        : json.groups
...
function applyGroupImport() {
  ...
  for (const g of groups) {
    if (g.kind === 'built-in') {
      ...
      const previous = customs.findIndex(
        (x) => x.id === g.id || (x.facility === g.facility && x.title === g.title)
      )
  ...
  if (customs.length > 50) {
    message('合并后超过 50 组，请选择替换导入', 'warning')
    return
  }
  ...
  hiddenGroupIds.value = replace
    ? [
        ...new Set([
          ...hidden,
          ...builtinGroups
            .filter((preset) => !groups.some((g) => g.id === preset.id))
  ...
  if (first) {
    facilityFilter.value = first.facility
...
try {
  rosterApprovals.value = JSON.parse(
    localStorage.getItem('mower-gacha-roster-approvals-v1') || '{}'
  )
} catch {
  /* LocalStorage may be unavailable */
}
const visiblePools = computed(() =>
  poolExpanded.value ? summary.value?.pools || [] : (summary.value?.pools || []).slice(0, 6)
)
const rosterApproved = computed(
  () =>
    !!(
      activeId.value &&
      rosterInfo.value?.available &&
...
const matchedRoster = computed(() => {
  ...
  return (rosterInfo.value?.operators || []).filter(
    (op) =>
      (!keyword ||
...
function approveRoster() {
  ...
  rosterApprovals.value[activeId.value] = rosterInfo.value.observed_at
  try {
    localStorage.setItem('mower-gacha-roster-approvals-v1', JSON.stringify(rosterApprovals.value))
  } catch {}
}
watch(activePane, (pane) => {
  if (pane === 'roster' && !rosterInfo.value) loadRoster()
})
...
const selectedSession = computed(() =>
  sessions.value.find(
    (s) => s.selected && s.selected.channel + ':' + s.selected.uid === activeId.value
  )
)
const categories = computed(() => [
  { label: '所有卡池类型', value: '' },
  ...Object.keys(summary.value?.categories || {}).map((key) => ({
...
function categoryLabel(id) {
  return (
    { normal: '常规寻访', classic: '中坚寻访', anniver_fest: '周年限定', summer_fest: '夏日限定' }[
      id
    ] || id
  )
}
...
function setCooldown(seconds) {
  ...
  try {
    sessionStorage.setItem(
      'mower-gacha-sms-cooldown',
      JSON.stringify({
        lastFour: phone.value.slice(-4),
        expires
      })
    )
  } catch {
    /* Timer still works if browser storage is disabled. */
  }
  restoreCooldown()
}
let cooldownTimer
...
function queryParams(extra = {}) {
  const params = {
    account_id: activeId.value,
    limit: 50,
    category: category.value,
    pool_id: poolId.value,
    search: search.value.trim(),
    new_only: newOnly.value ? '1' : '0',
    ...extra
  }
...
async function loadData() {
  ...
  try {
    ...
    if (requestNo === viewRequest) message(errorText(error), 'error')
  } finally {
    if (requestNo === viewRequest) loading.value = false
  }
}
...
async function login() {
  ...
  if (!/^1[3-9]\d{9}$/.test(phone.value)) {
    message('手机号格式不正确', 'warning')
    return
  }
  if (loginMode.value === 'sms' && !/^\d{4,8}$/.test(code.value)) {
    message('请输入正确的短信验证码', 'warning')
    return
  }
  if (loginMode.value === 'password' && !password.value) {
    message('请输入通行证密码', 'warning')
    return
  }
...
async function chooseRole(sessionId, role) {
  ...
  try {
    const res = await axios.post(
      api('/select-role'),
      { session_id: sessionId, uid: role.uid, channel: role.channel },
      writeOptions
    )
...
async function exportBackup() {
  ...
  try {
    const res = await axios.get(api('/export'), {
      params: { account_id: activeId.value },
      responseType: 'blob'
    })
    ...
    link.href = url
    link.download =
      (activeAccount.value?.channel || 'ark') +
      '-' +
      (activeAccount.value?.uid || 'records') +
      '-gacha.json'
    link.click()
    ...
    <div class="gacha-stack">
      ...
      <div class="gacha-toolbar">
        <n-select
          ...
          class="gacha-toolbar-account"
          placeholder="选择账号"
          size="medium"
        />
        <n-text v-else depth="3" class="gacha-toolbar-account">未添加账号</n-text>
        <n-button
          v-if="activePane === 'gacha'"
          class="gacha-toolbar-sync"
          type="primary"
          :loading="syncing"
          :disabled="!selectedSession"
          title="只在点击时向鹰角同步寻访记录"
          @click="sync"
        >
          同步寻访
        </n-button>
        <n-button
          ...
          title="只在点击时使用 Mower 当前森空岛账号同步"
          @click="refreshRemoteRoster"
        >
          同步干员
        </n-button>
        <template v-if="activePane === 'gacha'">
          <n-button
            secondary
            size="small"
            @click="loginExpanded = !loginExpanded"
          ...
          <n-text
            depth="3"
            class="gacha-toolbar-meta"
            :title="
              '最近同步：' +
              (activeAccount?.last_sync ? formatDate(activeAccount.last_sync * 1000) : '未同步')
            "
          >
            {{ activeAccount?.total || 0 }} 抽{{
              activeAccount && !selectedSession ? ' · 待授权' : ''
            }}
          </n-text>
        </template>
        <template v-else>
          <n-button
            ...
            :loading="rosterLoading"
            @click="loadRoster"
            title="只读取本机缓存，不请求森空岛"
            >读取缓存</n-button
          >
          <n-button
            v-if="!rosterApproved && rosterInfo?.available && activeAccount"
            secondary
            size="small"
            title="确认本机森空岛缓存属于所选账号，无需重新登录"
            @click="approveRoster"
            >确认归属</n-button
          >
          <n-text
      ...
      <div v-if="activePane === 'roster'" class="gacha-roster-container">
        <n-card key="roster-profile-card" title="干员档案" size="medium">
          <n-space vertical :size="12">
            <n-alert
              v-if="rosterNeedsSetup || rosterInfo?.available === false"
              type="warning"
              :bordered="false"
            >
              <n-space align="center" :wrap="true">
                <n-text>未读取到森空岛干员数据</n-text>
                <n-button text type="primary" @click="openSklandSettings"
                  >前往 Mower 设置 → 森空岛</n-button
                >
                <HelpText label="森空岛说明"
                  >请先在 Mower 设置配置森空岛，已有数据可直接读取缓存。</HelpText
                >
              </n-space>
            </n-alert>
            <template v-if="rosterInfo?.available">
              <n-text
                v-if="rosterInfo.unknown_id_count"
                depth="3"
                class="gacha-muted"
              ...
              <n-space align="center" :wrap="true">
                <n-input
                  v-model:value="rosterQuery"
                  clearable
                  placeholder="搜索名称或干员 ID，例如结城理"
              ...
              <n-space align="center" :wrap="true" class="gacha-roster-stats">
                <n-button
                  size="small"
                  :type="eliteFilter === 'all' ? 'primary' : 'default'"
                  :secondary="eliteFilter !== 'all'"
                  @click="eliteFilter = 'all'"
                  >全部 {{ rosterInfo.operator_count }}</n-button
                >
                <n-button
                  size="small"
                  :type="eliteFilter === 'e2_90' ? 'primary' : 'default'"
                  :secondary="eliteFilter !== 'e2_90'"
                  @click="eliteFilter = eliteFilter === 'e2_90' ? 'all' : 'e2_90'"
                >
                  精二90 {{ rosterStats.e2_90 }}
                </n-button>
                <n-button
                  size="small"
                  :type="eliteFilter === 'e2_60' ? 'primary' : 'default'"
                  :secondary="eliteFilter !== 'e2_60'"
                  @click="eliteFilter = eliteFilter === 'e2_60' ? 'all' : 'e2_60'"
                >
                  精二60—89 {{ rosterStats.e2_60 }}
                </n-button>
                <n-button
                  size="small"
                  :type="eliteFilter === 'e2_other' ? 'primary' : 'default'"
                  :secondary="eliteFilter !== 'e2_other'"
                  @click="eliteFilter = eliteFilter === 'e2_other' ? 'all' : 'e2_other'"
                >
                  精二1—59 {{ rosterStats.e2_other }}
                </n-button>
                <n-button
                  size="small"
                  :type="eliteFilter === 'e1' ? 'primary' : 'default'"
                  :secondary="eliteFilter !== 'e1'"
                  @click="eliteFilter = eliteFilter === 'e1' ? 'all' : 'e1'"
                >
                  精一 {{ rosterStats.e1 }}
                </n-button>
                <n-button
                  size="small"
                  :type="eliteFilter === 'e0' ? 'primary' : 'default'"
                  :secondary="eliteFilter !== 'e0'"
                  @click="eliteFilter = eliteFilter === 'e0' ? 'all' : 'e0'"
                >
                  精零 {{ rosterStats.e0 }}
                </n-button>
              </n-space>
              <div class="gacha-roster-grid">
                <div v-for="op in visibleRoster" :key="op.id" class="gacha-roster-op">
                  <div class="gacha-record-avatar" :class="'star-' + op.rarity">
                    <img
                      v-if="!avatarMissing.has(op.name)"
                      :src="avatarUrl(op.name)"
                      :alt="op.name"
                      loading="lazy"
                      @error="markMissing(op.name)"
                    />
                    <span v-else>{{ op.name.slice(0, 1) }}</span>
                  </div>
                  <div class="gacha-roster-name">
                    <strong :title="op.name">{{ op.name }}</strong>
                    <n-text depth="3" class="gacha-muted"
                      >{{ op.rarity }}★ · 精英{{ op.evolve_phase }} · Lv{{ op.level }}</n-text
                    >
                  </div>
                </div>
              </div>
              ...
              <n-button
                v-if="matchedRoster.length > rosterLimit"
                block
                secondary
                @click="rosterLimit += 48"
                >显示更多干员（{{ matchedRoster.length - rosterLimit }}）</n-button
              >
            </template>
          </n-space>
        </n-card>
        ...
        <n-card
          key="roster-groups-card"
          v-if="rosterInfo?.available"
          title="基建组合"
          size="medium"
        >
          <div class="gacha-group-stack">
            <div class="gacha-group-toolbar">
              ...
              <n-select
                ...
                size="medium"
                class="gacha-group-select"
                placeholder="选择组合"
              />
              <n-button size="small" type="primary" secondary @click="startGroup()">新建</n-button>
              <n-button v-if="activeGroup" size="small" secondary @click="startGroup(activeGroup)"
                >编辑</n-button
              >
              <n-popconfirm
                v-if="activeGroup"
                ...
                positive-text="删除"
                negative-text="取消"
                @positive-click="deleteGroup"
              >
                <template #trigger
                  ><n-button size="small" quaternary type="error">删除</n-button></template
                >
                删除此组合？
              </n-popconfirm>
              <n-button
              ...
              <n-button
                size="small"
                quaternary
                @click="groupImportInput?.click()"
                title="导入 JSON 组合文件，可选择合并或替换"
                >导入</n-button
              >
              <input
                ref="groupImportInput"
                type="file"
                accept=".json,.jsonc,application/json"
                class="gacha-hidden-input"
                @change="readGroupImport"
              />
              <n-button
                size="small"
                quaternary
                @click="exportGroups"
                title="导出可人工或 AI 修改的 JSON 组合文件"
                >导出</n-button
              >
              <HelpText label="分享组合">
            ...
            <n-card v-if="groupImportPending" size="small" title="导入预览">
              <n-space vertical :size="9">
                <n-text
                  >{{ groupImportPending.fileName }} ·
                  {{ groupImportPending.groups.length }} 组</n-text
                >
              ...
              <div v-if="groupMembers.length" class="gacha-group-grid">
                ...
                >
                  ...
                  <n-button
                    ...
                    quaternary
                    circle
                    class="gacha-group-remove"
                    :title="'从组合中移除 ' + op.name"
                    @click.stop="removeGroupMember(op.id)"
                    >×</n-button
                  >
                </div>
              </div>
            ...
            <n-card v-if="groupEditorOpen" title="编辑组合" size="small">
              <n-space vertical :size="10">
                <n-space align="center" :wrap="true">
                  <n-input
                    v-model:value="draftGroup.title"
                    class="gacha-group-edit-name"
                    placeholder="组合名称"
                    :maxlength="50"
                  />
                  <n-select
                    v-model:value="draftGroup.facility"
                ...
                <n-select
                  ...
                  :render-tag="renderGroupTag"
                />
                <n-text depth="3" class="gacha-muted">挂件 / 替补</n-text>
                <n-select
                  v-model:value="draftGroup.support"
                  :options="catalogOptions"
                  multiple
                  filterable
                  clearable
                ...
                />
                <n-input
                  v-model:value="draftGroup.note"
                  type="textarea"
                  :rows="2"
                  placeholder="备注（仅悬停查看，可留空）"
                  :maxlength="240"
                />
                <n-space>
        ...
        <n-space vertical :size="12">
          ...
          <div class="gacha-auth-fields">
            ...
            <template v-if="loginMode === 'sms'">
              ...
              <n-button
                :loading="sending"
                :disabled="sending || cooldown > 0 || !/^1[3-9]\d{9}$/.test(phone)"
                @click="sendSms"
              >
          ...
          <n-card
            v-if="roleChoices.length"
            size="small"
            :title="'已验证 ' + loginMask + ' · 请选择角色'"
          >
            <n-space vertical>
              <n-space
                v-for="role in roleChoices"
                :key="role.channel + ':' + role.uid"
                align="center"
              >
                <n-tag :type="role.channel === 'official' ? 'success' : 'info'">{{
                  serverText(role.channel)
                }}</n-tag>
                ...
                <n-button
                  ...
                  type="primary"
                  :loading="loginBusy"
                  @click="chooseRole(selectedSessionId, role)"
                  >连接此角色</n-button
                >
              </n-space>
            </n-space>
              ...
              <n-space vertical>
                <n-space v-for="session in sessions" :key="session.session_id" align="center">
                  <n-button
                    ...
                    secondary
                    :loading="loginBusy"
                    @click="chooseRole(session.session_id, role)"
                  >
                    {{ role.nickname }} · {{ serverText(role.channel) }}
                  </n-button>
                  <n-button size="small" quaternary type="error" @click="logout(session.session_id)"
                    >退出</n-button
                  >
                </n-space>
              </n-space>
      ...
      <template v-if="activePane === 'gacha' && summary && activeAccount">
        <div class="gacha-summary-grid">
          ...
          <n-card size="small"
            ><n-statistic label="不同干员" :value="summary.distinct_operators"
          /></n-card>
        </div>
        ...
        <n-card title="卡池概览" size="medium">
          ...
          <div
            v-if="summary.pools?.length"
            :class="['gacha-pool-grid', { 'gacha-pool-grid-expanded': poolExpanded }]"
          >
            <div
              v-for="pool in visiblePools"
              :key="pool.category + ':' + pool.pool_id"
              class="gacha-pool"
            >
              ...
              <div v-if="pool.six_operators.length" class="gacha-six-timeline">
                <div
                  ...
                  :title="
                    ...
                    op.pool_index +
                    ' 抽；' +
                    (op.interval_complete
                      ? '距上一六星 ' + op.interval_count + ' 抽'
                      : '本池首个已存档六星，前序可能缺失')
                  "
                  tabindex="0"
                  role="button"
                ...
                >
                  <div class="gacha-six-hit-head">
                    <div class="gacha-avatar-wrap">
                      <img
                        v-if="!avatarMissing.has(op.char_name)"
                        :src="avatarUrl(op.char_name)"
                        :alt="op.char_name"
                        loading="lazy"
                        @error="markMissing(op.char_name)"
                      />
                      <span v-else class="gacha-avatar-fallback">{{
                        op.char_name.slice(0, 1)
                      }}</span>
                    </div>
          ...
          >
            {{
              poolExpanded
                ? '收起，仅显示最近6池'
                : '查看其余 ' + (summary.pools.length - 6) + ' 个卡池'
            }}
          </n-button>
        </n-card>
          ...
          <div class="gacha-stars">
            <div v-for="star in starRows" :key="star.star" class="gacha-stars-row">
              ...
              <div class="gacha-stars-track">
                <div
                  :class="'gacha-stars-fill star-' + star.star"
                  :style="{
                    width: summary.total ? (star.count / summary.total) * 100 + '%' : '0%'
                  }"
                ></div>
              </div>
        ...
        <n-card title="寻访流水" size="medium">
          <n-space vertical>
            ...
            <n-space align="center">
              ...
              <n-text depth="3" class="gacha-muted"
                >默认只显示六星，其他星级可在筛选器中查看。</n-text
              >
            </n-space>
            <div class="gacha-record-list">
              <div
                v-for="(r, index) in records"
                :key="r.gacha_ts + ':' + r.position + ':' + index"
                class="gacha-record"
              >
                <div :class="'gacha-record-avatar star-' + r.rarity">
                  <img
                    v-if="!avatarMissing.has(r.char_name)"
                    :src="avatarUrl(r.char_name)"
                    :alt="r.char_name"
                    loading="lazy"
                    @error="markMissing(r.char_name)"
                  />
            ...
            <n-button v-if="more" block secondary :loading="loading" @click="loadMore"
              >加载更多</n-button
            >
          </n-space>
        </n-card>
...
.gacha-avatar-wrap {
  ...
  width: 49px;
  height: 49px;
  border: 1px solid rgba(204, 156, 65, 0.6);
  border-radius: 7px;
  overflow: visible;
  background: rgba(204, 156, 65, 0.1);
}
.gacha-avatar-wrap img {
  width: 100%;
  height: 100%;
  object-fit: cover;
  border-radius: 6px;
}
.gacha-avatar-fallback {
  display: grid;
  place-items: center;
  height: 100%;
}
...
.gacha-stars-row {
  display: grid;
  grid-template-columns: 48px minmax(0, 1fr) 40px 50px;
  align-items: center;
  gap: 12px;
}
.gacha-stars-track {
  background: var(--mower-control-surface, rgba(127, 127, 127, 0.15));
  height: 9px;
  border-radius: 10px;
  overflow: hidden;
}
...
.gacha-stars-fill.star-6 {
  background: #d2a054;
}
.gacha-stars-fill.star-5 {
  background: #ad89ca;
}
.gacha-stars-fill.star-4 {
  background: var(--mower-primary, #18a058);
}
.gacha-stars-fill.star-3 {
  background: #83939c;
}
.gacha-star-percent {
  text-align: right;
...
.gacha-record {
  ...
  align-items: center;
  gap: 13px;
  border-bottom: 1px solid var(--mower-divider, rgba(127, 127, 127, 0.14));
  padding: 10px 4px;
}
.gacha-record-avatar {
  width: 47px;
  height: 47px;
  flex: 0 0 47px;
  background: var(--mower-control-surface, rgba(127, 127, 127, 0.1));
  border-radius: 7px;
  display: grid;
  place-items: center;
  overflow: hidden;
}
.gacha-record-avatar.star-6 {
  border: 1px solid rgba(210, 160, 84, 0.7);
}
.gacha-record-avatar.star-5 {
  border: 1px solid rgba(173, 137, 202, 0.55);
}
.gacha-record-avatar img {
  display: block;
  width: 100%;
  height: 100%;
  object-fit: cover;
...
.gacha-pool {
  ...
  min-width: 0;
  border-radius: 10px;
  border: 1px solid var(--mower-divider, rgba(120, 120, 120, 0.16));
  background: var(--mower-control-surface, rgba(127, 127, 127, 0.035));
}
.gacha-six-timeline {
  display: grid;
...
.gacha-six-hit:focus-visible {
  outline: 2px solid var(--mower-primary, #18a058);
  outline-offset: 2px;
}
.gacha-six-hit-target {
  border-color: var(--mower-primary, #18a058);
  background: var(--mower-primary-block, rgba(24, 160, 88, 0.08));
}
.gacha-six-hit-head {
  display: flex;
  align-items: center;
...
.gacha-six-hit .gacha-avatar-wrap {
  flex: 0 0 44px;
  width: 44px;
  height: 44px;
}
...
.gacha-roster-op {
  border: 1px solid var(--mower-divider, rgba(120, 120, 120, 0.15));
  border-radius: 8px;
  padding: 9px;
  display: flex;
  gap: 9px;
...
.gacha-toolbar {
  ...
  flex-wrap: wrap;
  padding: 9px 12px;
  border: 1px solid var(--mower-divider, rgba(120, 120, 120, 0.14));
  border-radius: 10px;
  min-width: 0;
}
.gacha-toolbar-account {
  flex: 1 1 260px;
  max-width: 480px;
  min-width: 190px;
}
...
.gacha-group-member-missing {
  border-style: dashed;
}
.gacha-group-member-missing .gacha-group-avatar {
  opacity: 0.42;
...
.gacha-group-member:hover .gacha-group-remove,
.gacha-group-remove:focus-visible {
  opacity: 1;
}
.gacha-group-avatar {
  width: 51px;
...
.gacha-option-avatar {
  ...
  border-radius: 4px;
  object-fit: cover;
  background: var(--mower-control-surface, rgba(127, 127, 127, 0.08));
}
.gacha-option-tag {
  max-width: 100%;
}
...
@media (max-width: 550px) {
  .gacha-roster-container .n-radio-group.n-radio-group--button-group {
    display: flex;
    flex-wrap: wrap;
    max-width: 100%;
    width: 100%;
  ...
  .gacha-roster-container .n-radio-button {
    flex: 0 0 auto;
  }
  .gacha-toolbar-account {
    min-width: 100%;
