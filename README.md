...
**Mower 设置 → 配置导出与导入** 位于「进程操作」板块上方，支持导出当前实例的全部配置，包括所有排班与周计划、专精计划和训练员配置、共享网络设置等。导入时保留当前管理页面端口、访问令牌和网络代理，其余配置按备份恢复；导入前会自动备份现有配置。范围与操作说明见 [配置导出与导入](doc/config-backup.md)。
